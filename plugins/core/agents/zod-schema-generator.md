...
## Stack
- **Validation**: Zod — check `package.json` for the installed Zod major and write to that version's API (error-message and `z.record`/`z.string` signatures differ between majors)
...
1. Check the generated API types — if the shape is an API response, derive from them (`z.ZodType<Entity>` or `satisfies`) rather than redeclaring
...
// Enum from API — mirror the values the generated types declare
...
2. Run the project's typecheck with the detected manager (`<pm> run typecheck` if the script exists, else `<pm> exec tsc --noEmit`). If a test file exists for the schema or its form, also run `<pm> exec vitest run <that file>`.
